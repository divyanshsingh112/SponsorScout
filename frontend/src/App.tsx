...
// Views
const CheckoutInterstitial = lazy(() => import('./components/CheckoutInterstitial'));
const ClaimFallback = lazy(() => import('./components/ClaimFallback'));
...
DemosSection.displayName = 'DemosSection';

const FounderStorySection = React.memo(() => {
  return (
...
function App() {
  ...
  // Unified Wizard Input State synchronized to prevent fragmented rendering
  const [wizardValues, setWizardValues] = useState<any>(initialValues);
  ...
  const handleUnlock = () => {
    ...
    // Save form values
    localStorage.setItem('pending_pdf_channel_data', JSON.stringify(wizardValues));
  ...
  const simulatePayment = async () => {
    try {
      ...
      // Start download
      window.location.href = `${import.meta.env.VITE_API_BASE_URL}/api/download-kit/${wizardValues.channelId}`;
    } catch (err: any) {
      setError('Payment verification failed.');
    }
  };

  // Checkout screen (lazy loaded)
  ...
  if (view === 'claim-fallback') {
    return (
      <div className="min-h-screen bg-slate-950 text-white font-sans flex flex-col items-center justify-center">
        <Suspense fallback={<div className="text-slate-400 font-sans">Loading Manual Claim Form...</div>}>
          <ClaimFallback 
            onBack={() => setView('home')} 
            pendingChannelId={wizardValues.channelId || undefined} 
          />
        </Suspense>
      </div>
    );
                    ...
                    <div className="bg-slate-900/50 border border-slate-800/50 rounded-2xl p-3 flex items-start gap-2.5">
                      ...
                      <div>
                        ...
                        <div className="text-slate-200 font-bold text-sm truncate">
                          {wizardValues.platform === 'instagram' 
                            ? (channelData.topLocation || wizardValues.topLocation) 
                            : (channelData.audienceGeo || wizardValues.audienceGeo)}
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="bg-slate-900/60 rounded-3xl p-6 md:p-8 border border-slate-800 text-center relative overflow-hidden font-sans">
                    ...
                    {paymentStatus === 'success' && (
                      <div className="mt-6 animate-in fade-in zoom-in duration-500">
                        <button
                          onClick={() => window.location.href = `${import.meta.env.VITE_API_BASE_URL}/api/download-kit/${wizardValues.channelId}`}
